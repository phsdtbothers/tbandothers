...
extract_edcs <- function(bq_project, bq_edcs_dataset, bq_disease, run_date=Sys.Date()) {
  ...
  schema_pat <- bigrquery::bq_table_fields(bigrquery::bq_table(bq_project, bq_edcs_dataset, paste0(bq_disease, '_view_', date_info$current_dateshard))) %>% sapply(function(x) x$name)
  schema_lab <- bigrquery::bq_table_fields(bigrquery::bq_table(bq_project, bq_edcs_dataset, paste0(bq_disease, '_view_lab_', date_info$current_dateshard))) %>% sapply(function(x) x$name)

  schema_lab_only <- schema_lab[!schema_lab %in% schema_pat]

  # call from bigquery, get all pat fields, then new lab fields
  query <- paste0("SELECT pat.*, ", paste0('lab.', schema_lab_only, collapse=','), " FROM ", bq_edcs_dataset, ".", bq_disease, "_view_", date_info$current_dateshard, " pat LEFT JOIN ", bq_edcs_dataset, ".", bq_disease, "_view_lab_", date_info$current_dateshard, " lab ON pat.case_id = lab.case_id WHERE pat.last_modified_date BETWEEN '", date_info$previous_start, "' AND '", date_info$previous_end, "' OR lab.last_modified_date BETWEEN '", date_info$previous_start, "' AND '", date_info$previous_end, "'")
